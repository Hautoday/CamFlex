...
public class LostActivity extends AppCompatActivity {
    ...
    private void loadProducts() {
        ...
        apiService.getProducts("분실물").enqueue(new Callback<List<Product>>() {
            @Override
            public void onResponse(Call<List<Product>> call, Response<List<Product>> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (response.body().isEmpty()) {
                        Toast.makeText(LostActivity.this, "해당 카테고리의 상품이 없습니다.", Toast.LENGTH_SHORT).show();
                        return;
                    }

                    adapter = new ProductAdapter(LostActivity.this, response.body());
                    recyclerView.setAdapter(adapter);
                } else {
                    Toast.makeText(LostActivity.this, "데이터 로드 실패 (Code: " + response.code() + ")", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Product>> call, Throwable t) {
                Toast.makeText(LostActivity.this, "서버 연결 실패", Toast.LENGTH_LONG).show();
