...
public class EtcActivity extends AppCompatActivity {
    ...
    private void loadProducts() {
        ...
        apiService.getProducts("기타").enqueue(new Callback<List<Product>>() {
            @Override
            public void onResponse(Call<List<Product>> call, Response<List<Product>> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (response.body().isEmpty()) {
                        Toast.makeText(EtcActivity.this, "해당 카테고리의 상품이 없습니다.", Toast.LENGTH_SHORT).show();
                        return;
                    }

                    adapter = new ProductAdapter(EtcActivity.this, response.body());
                    recyclerView.setAdapter(adapter);
                } else {
                    Toast.makeText(EtcActivity.this, "데이터 로드 실패 (Code: " + response.code() + ")", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Product>> call, Throwable t) {
                Toast.makeText(EtcActivity.this, "서버 연결 실패", Toast.LENGTH_LONG).show();
